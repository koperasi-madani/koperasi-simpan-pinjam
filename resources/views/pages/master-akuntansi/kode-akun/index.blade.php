                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <select name="id_induk" id="id_induk" class="form-control">
                                        @foreach ($kode as $item)
                                            <option value="{{ $item->id }}" {{ old('id_induk') == $item->id ? 'selected' : '' }}>{{ $item->kode_induk }} -- {{ $item->nama }}</option>
                                        @endforeach
                                    </select>
                                    @error('id_induk')
                                ...
                                <tbody>
                                    @forelse ($data as $item)
                                        <tr>
                                            ...
                                            <td>{{ ucwords($item->nama_akun) }}</td>
                                            <td>{{ ucwords($item->jenis) }}</td>
                                            <td>{{ \Carbon\Carbon::parse($item->created_at)->translatedFormat('d F Y ')  }}</td>
                                            <td>
                                                <div class="d-flex justify-content-start">
                                                    <div class="mx-2">
                                                        <a href="{{ route('kode-akun.edit',$item->id) }}" class="btn btn-sm font-sm rounded btn-brand"> <i class="material-icons md-edit"></i> Edit </a>
                                                    </div>
                                                    <form action="{{ route('kode-akun.destroy',$item->id) }}" class="p-0 m-0" method="POST" onsubmit="return confirm('Move data to trash? ')">
                                                        @method('delete')
                                                        @csrf
